...
const DEFAULT_CAVEMAN_TRANSCRIPT = [
  ...
  {
    role: "bot",
    text: "If you want cave-mail portrait, enter the Gift Rite and leave an email for the message stone."
  }
];
...
function bindEvents() {
  ...
  els.openCavemanBot.addEventListener("click", () => {
    els.cavemanSection?.scrollIntoView({
      behavior: "smooth",
      block: "start"
    });
  });
  ...
  document.querySelectorAll("[data-caveman-prompt]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      handleCavemanInput(prompt);
    });
  });
  els.cavemanSend.addEventListener("click", () => {
    handleCavemanInput(els.cavemanInput.value);
  });
...
function handleCavemanInput(rawMessage) {
  ...
  const reply = buildCavemanReply(message, {
    discoveredName,
    discoveredEmail
  });
  state.caveman.transcript = [...state.caveman.transcript, { role: "bot", text: reply }].slice(-18);
  if (discoveredEmail) {
    state.caveman.mailStatus = `Email remembered for ${state.caveman.email}. Open Gift Rite when you want the portrait message.`;
  }
...
function buildCavemanReply(message, discoveries = {}) {
  ...
  if (/(bitcoin|btc|ethereum|eth|solana|crypto|coin|market|altcoin)/.test(lower)) {
    ...
    response += "Portrait ritual lives in Gift Rite. Put name and email there, then Grug prepares a mail stone with the caveman image.";
  ...
  if (!state.caveman.name && !discoveries.discoveredName) {
    ...
    response += " If you want the caveman portrait email, leave your address in Gift Rite.";
  }
...
function bindSceneMotion() {
  ...
  const pulseScene = (event) => {
    ...
    rootStyle.setProperty("--scene-zoom", String(Number.parseFloat(getComputedStyle(document.documentElement).getPropertyValue("--scene-zoom")) + 0.03));
